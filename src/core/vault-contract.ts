...
import { OKF_VERSION } from "./okf.js";
...
type: overview
title: Overview
description: Starting page for durable source-grounded wiki synthesis.
timestamp: ${today}
status: draft
created: ${today}
updated: ${today}
...
This repository is a local-first LLM wiki vault and an OKF v0.1 bundle.

## OKF Mapping

- \`wiki/\` is the OKF bundle root.
- \`wiki/index.md\` is the root OKF index and declares \`okf_version: "0.1"\`.
- \`wiki/log.md\` is the bundle log. It has no frontmatter and uses \`## YYYY-MM-DD\` date headings.
- Other markdown files under \`wiki/\` are OKF concept pages.
- Concept pages use frontmatter fields \`type\`, \`title\`, \`description\`, and \`timestamp\`.
- Reserved nested \`index.md\` and \`log.md\` files are not generated by default.
...
- Concept pages use YAML frontmatter.
...
8. Keep generated pages OKF-conformant: concept pages need \`type\`, \`title\`, \`description\`, and \`timestamp\`; log entries go under \`## YYYY-MM-DD\` headings.
...
function template(type: string, today: string): string {
  const title = `${type.charAt(0).toUpperCase() + type.slice(1)} Title`;
  return `---
type: template
title: ${title}
description: Template for ${type} pages.
timestamp: ${today}
templateFor: ${type}
status: draft
created: ${today}
